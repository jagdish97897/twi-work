          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {currentIndents.map((indent) => (
                <tr key={indent._id}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{indent.indentNo}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{new Date(indent.date).toLocaleDateString()}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{indent.customer}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    <button
                      onClick={() => handleEdit(indent._id)}
                      className="text-blue-600 hover:text-blue-900 mr-4"
                    >
                      <FaEdit className="inline-block" /> Edit
                    </button>
                    <button
                      onClick={() => handlePrint(indent._id)}
