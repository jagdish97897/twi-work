...
const JobOrder = () => {
  ...
  const fetchConsignorDetails = async (consignorName) => {
    try {
      const response = await axios.get(`http://localhost:5000/getcustomer2`, { params: { name: consignorName } });
      const consignor = response.data[0];
      if (consignor) {
        setJobOrder((prevOrder) => ({
          ...prevOrder,
          consignorGSTIN: consignor.gst.GSTIN,
          consignorAddress: consignor.contact.Address
        }));
      }
    } catch (error) {
  ...
  const fetchConsigneeDetails = async (consigneeName) => {
    try {
      const response = await axios.get(`http://localhost:5000/getcustomer2`, { params: { name: consigneeName } });
      const consignee = response.data[0];
      if (consignee) {
        setJobOrder((prevOrder) => ({
          ...prevOrder,
          consigneeGSTIN: consignee.gst.GSTIN,
          consigneeAddress: consignee.contact.Address
        }));
      }
    } catch (error) {
      ...
      <form onSubmit={handleSubmit}>
         ...
         <div className='sm:flex sm:flex-wrap gap-4'>
          ...
          <div className="mb-4">
            ...
            <datalist id="consignorSuggestions">
              {consignorData.map((consignor) => (
                <option key={consignor.contact.Name} value={consignor.contact.Name}>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              id="consignorGSTIN"
              name="consignorGSTIN"
              value={jobOrder.consignorGSTIN}
              readOnly
              className="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm bg-gray-100 sm:text-sm"
            />
          </div>
          <div className="mb-4">
            ...
            <input
              ...
              value={jobOrder.consignorAddress}
              readOnly
              className="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm bg-gray-100 sm:text-sm"
            />
          </div>
          ...
          <div className="mb-4">
            ...
            <datalist id="consigneeSuggestions">
              {consigneeData.map((consignee) => (
                <option key={consignee.contact.Name} value={consignee.contact.Name}>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              value={jobOrder.consigneeAddress}
              readOnly
              className="mt-1 p-2 block w-full border border-gray-300 rounded-md shadow-sm bg-gray-100 sm:text-sm"
            />
          </div>
        ...
        <Tabs className="bg-white mt-4 rounded-lg shadow-lg">
          <TabList className="flex flex-wrap border-b border-gray-200 bg-indigo-100 rounded-t-lg">
            <Tab
             className="py-2 px-4 cursor-pointer hover:bg-gray-100 w-full sm:w-auto text-indigo-800"
...
//       setIndentItems(indent.additem);
//       setFromOptions([...new Set(indent.additem.map(item => item.from))]);
//       setToOptions([]);
